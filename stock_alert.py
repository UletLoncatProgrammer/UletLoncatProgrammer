...
class Thresholds:
    ...
    ma_short: int = 20
    ma_long: int = 50
    min_auto_band_pct: float = 1.0  # if a stock's auto buy/sell zone is narrower
    # than this % of price, treat it as flat/frozen (e.g. sitting on IDX's price
    # floor) and skip the auto buy/sell target alerts for it — a near-zero-width
    # zone isn't a meaningful signal, it just means the stock hasn't moved.
...
def compute_bollinger(close: pd.Series, window: int = 20, num_std: float = 2.0) -> tuple[float | None, float | None]:
    """Adaptive buy/sell zone from each stock's OWN recent volatility.
    Recomputed fresh every run since fetch_history pulls current data daily.
    Lower band = statistically cheap (auto buy target); upper band = statistically
    expensive (auto sell target). Widens/narrows automatically per stock."""
...
def analyze(stock: Stock, history: pd.DataFrame, thresholds: Thresholds) -> tuple[dict, list[tuple[str, str]]]:
    ...
    auto_buy, auto_sell = compute_bollinger(close)

    # Guard against a flat/frozen stock (e.g. sitting on IDX's price floor,
    # like a suspended or near-worthless stock at Rp50): if the calculated
    # auto zone is essentially zero-width, it's not a meaningful buy/sell
    # signal, so drop it and let other alert types (RSI, volatility, cross)
    # still work normally.
    if auto_buy is not None and auto_sell is not None:
        band_pct = (auto_sell - auto_buy) / last_close * 100 if last_close else 0
        if band_pct < thresholds.min_auto_band_pct:
            auto_buy, auto_sell = None, None

    # Manual target in config.json always wins if set; otherwise fall back to
    # the adaptive Bollinger target, recalculated fresh every day.
    effective_buy = stock.buy_below if stock.buy_below is not None else auto_buy
    effective_buy_is_auto = stock.buy_below is None
    effective_sell = stock.sell_above if stock.sell_above is not None else auto_sell
    effective_sell_is_auto = stock.sell_above is None

    alerts: list[tuple[str, str]] = []

    if effective_buy is not None and last_close <= effective_buy:
        tag = "auto" if effective_buy_is_auto else "your"
        alerts.append(("buy_target", f"{stock.ticker} at {last_close:.2f} has dropped to/below {tag} buy target {effective_buy:.2f}"))

    if effective_sell is not None and last_close >= effective_sell:
        tag = "auto" if effective_sell_is_auto else "your"
        alerts.append(("sell_target", f"{stock.ticker} at {last_close:.2f} has risen to/above {tag} sell target {effective_sell:.2f}"))
    ...
    summary = {
        "ticker": stock.ticker, "close": last_close, "change_pct": change_pct, "rsi": last_rsi,
        "buy_target": effective_buy, "sell_target": effective_sell,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    ticker_alerts: list[tuple[str, list[tuple[str, str]]]] = []
    print(f"{'Ticker':<10}{'Close':>10}{'Chg%':>8}{'RSI':>8}{'BuyTgt':>10}{'SellTgt':>10}")
    for stock in stocks:
        try:
        ...
        bt = f"{summary['buy_target']:.2f}" if summary['buy_target'] is not None else "-"
        st = f"{summary['sell_target']:.2f}" if summary['sell_target'] is not None else "-"
        print(f"{summary['ticker']:<10}{summary['close']:>10.2f}{summary['change_pct']:>7.2f}%{summary['rsi']:>8.1f}{bt:>10}{st:>10}")
